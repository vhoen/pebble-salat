...
const backgroundColor = render.makeColor(255, 255, 255);
const titleBarColor = render.makeColor(0, 0, 0);
const prayerUpcomingColor = render.makeColor(0, 0, 0);
const prayerPastColor = render.makeColor(150, 150, 150);
const titleTextColor = render.makeColor(255, 255, 255);
...
function drawHourglass(x, y, color) {
	const scale = 1;
	for (let row = 0; row < HOURGLASS.length; row++) {
...
function draw() {
	...
	render.fillRectangle(titleBarColor, 10, 10, render.width - 20, 22);

	drawBitmapText("Salat", 14, 14, titleTextColor);
	if (state.loading)
		drawHourglass(render.width - 22, 17, titleTextColor);
	...
	for (let i = 0; i < prayerRows.length; i++) {
		...
		const color = status.isPast ? prayerPastColor : prayerUpcomingColor;
...
function readIntValue(payload, key) {
	let value = null;
...
const message = new Message({
	...
	onWritable() {
		...
		state.loading = true;
		draw();
		message.write(new Map([["requestFetch", 1]]));
	},
	onReadable() {
		...
		const fetching = readIntValue(payload, "fetching");
		if (fetching === 1)
			state.loading = true;
		else if (fetching === 0)
			state.loading = false;
